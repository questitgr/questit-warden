...
define( 'QUESTIT_WARDEN_VERSION', '3.5.4' );
...
class Questit_Warden {
    ...
    private function __construct() {
        ...
        if ( is_admin() ) {
            ...
            add_action( 'admin_post_questit_warden_clear_update_cache', array( $this, 'handle_clear_update_cache' ) );
    ...
    public function handle_clear_update_cache() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'questit_warden_clear_update_cache' ) ) {
        ...
        wp_safe_redirect( add_query_arg(
            array( 'page' => 'questit-warden', 'cache_cleared' => '1' ),
    ...
    public function admin_notices() {
        ...
        if ( isset( $_GET['cache_cleared'] ) && $_GET['cache_cleared'] === '1' ) {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Cache καθαρίστηκε! Πήγαινε στο Dashboard → Updates και πάτα "Επανέλεγχος".', 'questit-warden' ) . '</p></div>';
        }
    }
    ...
    public function render_settings_page() {
        ...
        <div class="wrap">
            ...
            <div style="background: #fff; padding: 20px; border: 1px solid #ccd0d4; margin-top: 20px; max-width: 700px;">
                ...
                <p style="color: #666;">
                    Ο Warden ελέγχει το GitHub για νέες εκδόσεις κάθε <strong>12 ώρες</strong>. 
                    Αν μόλις έβγαλες νέο release και θέλεις να εμφανιστεί αμέσως, καθάρισε το cache.
                </p>
                <?php
                $github_cache = get_transient( 'questit_warden_github_release' );
                $cached_version = $github_cache ? ltrim( $github_cache->tag_name, 'v' ) : null;
                ?>
                <?php if ( $cached_version ): ?>
                    <p style="color: #666;">
                        Cache: έκδοση <strong><?php echo esc_html( $cached_version ); ?></strong>
                        <?php
                        $timeout_key = '_transient_timeout_questit_warden_github_release';
                        $expires = get_option( $timeout_key );
                        if ( $expires ) {
                            $mins = round( ( $expires - time() ) / 60 );
                            echo '<span style="color: #999;">— λήγει σε ' . esc_html( $mins ) . ' λεπτά</span>';
                        }
                        ?>
                    </p>
                <?php else: ?>
                    <p style="color: #999;"><em>Κανένα cache αποθηκευμένο.</em></p>
                <?php endif; ?>
                
                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top: 10px;">
                    <input type="hidden" name="action" value="questit_warden_clear_update_cache">
                    <?php wp_nonce_field( 'questit_warden_clear_update_cache' ); ?>
    ...
    public function register_rest_endpoints() {
        ...
        register_rest_route( 'questit-warden/v1', '/force-update-check', array(
            'methods'             => 'POST',
            'callback'            => array( $this, 'handle_force_update_check' ),
            'permission_callback' => '__return_true',
        ) );
    }
    
    /**
     * Handle force update check request from Watchtower
     * Clears GitHub release cache so WordPress picks up the new version immediately
     * Uses same security as /trigger endpoint
     */
    public function handle_force_update_check( $request ) {
        // Reuse same auth logic as trigger (rate limit, timestamp, HMAC)
        $auth = $this->verify_watchtower_request( $request );
        if ( is_wp_error( $auth ) ) {
            return $auth;
        }
        
        // Clear GitHub release transient so next check hits API fresh
        delete_transient( 'questit_warden_github_release' );
    ...
    /**
     * Shared authentication logic for Watchtower requests
     * Used by both /trigger and /force-update-check
     */
    private function verify_watchtower_request( $request ) {
        ...
        if ( abs( time() - intval( $data['timestamp'] ) ) > 300 ) {
            return new WP_Error( 'invalid_timestamp', 'Request expired', array( 'status' => 401 ) );
        }
        ...
        if ( empty( $this->watchtower_url ) || empty( $this->api_key ) ) {
            return new WP_Error( 'not_configured', 'Warden not configured', array( 'status' => 500 ) );
        }
        
        // Verify Watchtower URL
        if ( ! $this->urls_match( $this->watchtower_url, $data['watchtower_url'] ) ) {
            return new WP_Error( 'invalid_watchtower', 'Watchtower URL mismatch', array( 'status' => 401 ) );
        }
        
        // Verify HMAC
        // Normalize site_url the same way Watchtower does (remove www, trailing slash)
        $site_url = rtrim( site_url(), '/' );
        $site_url = preg_replace( '#^(https?://)www\.#i', '$1', $site_url );
